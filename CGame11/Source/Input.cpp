...
#include <sstream>
 ...
 bool Input::KeyStopped = false;
 bool Input::MouseStopped = false;
...
void* Input::ProcessKeyboardThread(void* arg)
{
	...
	// there may be more than one instance of keys called so we must make sure we are able to close down a thread
// that might require waiting to confirm so these flags will let us know when the threads are stopped and we can delete the instance
	FILE *fp;
	...
	if (fp != NULL)
	{
		...
		while (((Input*)arg)->kQuit == false) // kQuit is set to false by the init
		{
			...
			{
				((Input*)arg)->Keys[ev.code] = ev.value>0 ? TRUE : FALSE; 
	...
	printf("  err %d \n", errno); // we closed so let's see the error if there is one
	KeyStopped = true;
	pthread_exit(NULL);
}
...
//mice don't usually provide any issues uncomment if you need a mouse
...
/* 
 ...
 sudo chmod  a+r /dev/input/ * (assuming your input event files are there) << a space was added beteen / and * to prevent an annoying and misplaced warning
...
*/
	...
	// If this is your case, then just change this variable to event-mouse. We need to work on a way to make this autodetect
